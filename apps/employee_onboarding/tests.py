...
class OnboardingModuleTests(APITestCase):
    ...
    def test_bitrix_webhook_by_id_success(self, mock_get_user_detail, mock_email_delay):
        ...
        # Also check crm.id as requested
        payload_crm = {'crm.id': '10'}
        res_crm = self.client.post(url, payload_crm, format='json')
        self.assertEqual(res_crm.status_code, status.HTTP_202_ACCEPTED)
        self.assertEqual(res_crm.json()['status'], 'queued')
        ...
        mock_email_delay.assert_called_with(mock_user)
        ...
        self.assertEqual(res_nested.status_code, status.HTTP_202_ACCEPTED)
